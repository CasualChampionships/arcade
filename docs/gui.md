...
> [!NOTE]
> If you're using these GUI components in a minigame, the minigame will handle all the player management for you, see the section on [Minigame GUI](./minigames/gui.md) for more information.
...
The `TeamListEntries` however requires some resource packs which are provided by Arcade, namely a resource pack to player heads, hide player heads, player ping, and a negative padding resource pack. More information about these packs in the [Resources Section](resources.md)
