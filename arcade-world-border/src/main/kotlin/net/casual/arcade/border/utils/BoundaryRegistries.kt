...
public object BoundaryRegistryKies: RegistryKeySupplier(ArcadeUtils.MOD_ID) {
...
public object BoundaryRegistries: RegistrySupplier() {
    public val BOUNDARY_RENDERER_FACTORY: Registry<MapCodec<out BoundaryRenderer.Factory>> = this.create(BoundaryRegistryKies.BOUNDARY_RENDERER_FACTOR, BoundaryRenderer.Factory::bootstrap)
    public val BOUNDARY_SHAPE: Registry<MapCodec<out BoundaryShape>> = this.create(BoundaryRegistryKies.BOUNDARY_SHAPE, BoundaryShape::bootstrap)
    public val PARTICLE_RENDER_OPTIONS: Registry<ParticleRenderOptions> = this.create(BoundaryRegistryKies.PARTICLE_RENDER_OPTIONS) { ParticleRenderOptions.bootstrap() }
    public val MODEL_BOUNDARY_RENDER_OPTIONS: Registry<AxisAlignedModelRenderOptions> = this.create(BoundaryRegistryKies.MODEL_RENDER_OPTIONS) { AxisAlignedModelRenderOptions.bootstrap() }
}